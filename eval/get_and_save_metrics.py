...
parse_functions = {
    ...
    "countdown": parse_countdown_answers,
    "sudoku": parse_sudoku_answers,
}


def aggregate_results(directory=".", task='gsm8k', diffusion_steps=None, gen_length=None):
    ...
    for json_file in json_files:
        ...
        detailed_results,
        effective_tokens,
        ) = parse_functions[task](json_path=json_file)

        total_correct += correct
        total_processed += processed
        total_effective_tokens += effective_tokens
        total_questions.extend(detailed_results)

        print(f"Correct: {correct}, Processed: {processed}, Effective Tokens: {effective_tokens}")
    ...
    avg_effective_tokens = (
        total_effective_tokens / total_processed if len(total_questions) > 0 else 0
    )
    ...
    print(row_format.format(f"checkpoint-{checkpoint_id}", accuracy, avg_effective_tokens))
...
if __name__ == "__main__":
    ...
    if not checkpoint_ids:
        # list and extract checkpoint_ids from the directory
        checkpoint_ids = [int(checkpoint.split("-")[-1]) for checkpoint in glob.glob(os.path.join(checkpoints_dir, "checkpoint-*"))]
    ...
    for gen_length in gen_lengths:
        ...
        for checkpoint_id, checkpoint in zip(checkpoint_ids, checkpoints):
            ...
            # Initialize checkpoint_id if not exists
            if checkpoint_id not in all_metrics:
                all_metrics[checkpoint_id] = {}
            
            # Store metrics for this gen_length
            all_metrics[checkpoint_id][gen_length] = {
                "accuracy": metrics["accuracy"],
                "avg_effective_tokens": metrics["avg_effective_tokens"]
            }

        # Best accuracy checkpoint
        best_accuracy_checkpoint_id = max(all_metrics, key=lambda x: all_metrics[x][gen_length]["accuracy"])
        best_accuracy = all_metrics[best_accuracy_checkpoint_id][gen_length]["accuracy"]
        ...
        # For this gen_length and diffusion_steps, find the checkpoint with the best accuracy and save the metrics to a JSON file
        best_accuracy_checkpoint_id = max(all_metrics, key=lambda x: all_metrics[x][gen_length]["accuracy"])
        best_accuracy = all_metrics[best_accuracy_checkpoint_id][gen_length]["accuracy"]
        print(f"Best accuracy checkpoint for gen_length {gen_length} and diffusion_steps {diffusion_steps}: {best_accuracy_checkpoint_id}, accuracy: {best_accuracy}")

...
@dataclass
class EPSAConfig(TrainingArguments):
    r"""
    ...
    terminate_at_last_non_eos: bool = field(
        default=False,
        metadata={"help": "Whether to choose the time steps only among and up to the last non-EOS steps."},
    )
    ...
    normalize_returns: bool = field(
        default=False,
        metadata={"help": "Whether to normalize the returns by 1 + lambda1."},
    )
    is_lam_scheduling: bool = field(
        default=False,
        metadata={"help": "is_lam_scheduling"},
    )
    ...
    time_dependent_lambda: bool = field(
        default=False,
        metadata={"help": "Whether to use standard GRPO returns. If False, we use stepwise advantages to compute the returns."},
    )
    model_type: str = field(
        default="llada", metadata={"help": "Model family: 'llada' or 'dream'"}
    )

    use_logprob_intermediate_reward: bool = field(
        default=False,
        metadata={"help": "Replace sparse completion reward at intermediate steps with "
                          "log p(a* | prompt, x0^t) read from generation logits. "
                          "No extra forward passes. LLaDA only."},
    )
    gt_answer_col: str = field(
        default="answer",
        metadata={"help": "Dataset column name holding the ground-truth answer string. "
                          "Used when use_logprob_intermediate_reward=True. "
                          "GSM8K: 'answer', MATH: 'answer', Sudoku: 'solution'."},
    )
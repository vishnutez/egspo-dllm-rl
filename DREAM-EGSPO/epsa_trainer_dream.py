...
class EPSATrainer(GRPOTrainer):
    ...
    def __init__(
        ...
        self.model_type = getattr(self.args, "model_type", "llada").lower()
        self._mask_id = 151666 if self.model_type == "dream" else 126336
        self._logit_shift = (self.model_type == "dream")
        print(f"EPSATrainer: model_type={self.model_type}, mask_id={self._mask_id}, logit_shift={self._logit_shift}", flush=True)
        

    def _apply_logit_shift(self, logits):
        """Dream's causal forward pass needs a right-shift so logit[i] predicts token[i]."""
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if self.args.is_lam_scheduling and self._step > 0 and self._step % 1000 == 0:
            self.args.lambda1 *= 0.5
            print('lambda is halfed, the current value is: ', float(self.args.lambda1))
        if return_outputs:
                    ...
                    if hasattr(torch.cuda, "amp") and hasattr(torch.cuda.amp, "autocast"):
                        with torch.cuda.amp.autocast(enabled=self.args.fp16):
                            ...
                            if cfg_scale > 0.0:
                                ...
                                logits = self._apply_logit_shift(model(x_).logits)
                                logits, un_logits = torch.chunk(logits, 2, dim=0)
                                logits = un_logits + (cfg_scale + 1) * (logits - un_logits)
                            else:
                                logits = self._apply_logit_shift(model(x).logits)
    ...
    def get_logits(self, model, batch, prompt_index, cfg_scale, mask_id):
        ...
        input = batch
        logits = self._apply_logit_shift(model(input).logits)
    ...
    def _get_per_token_logps_unbiased(self, model, trajectory_ids, eval_time_steps, get_all_tokens_logps=False):
        ...
        if eval_time_steps.dim() == 1:
            ...
            for i in range(num_steps):
                ...
                pred_logits_all = self._apply_logit_shift(model(curr_state).logits) # [batch_size, seq_len, vocab_size]
        ...
        # Handle 2D case (different eval_time_steps for each batch)
            ...
            for b in range(batch_size):
                ...
                for i in range(num_steps):
                    ...
                    pred_logits_all = self._apply_logit_shift(model(curr_state).logits)  # [1, seq_len, vocab_size]
                    pred_logits = pred_logits_all[positions]  # [positions.sum(), vocab_size]
